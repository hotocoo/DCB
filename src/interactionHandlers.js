...
export async function updateInventoryEmbed(interaction, itemsByType, inventoryValue) {
  ...
  if (!embed) return;
  const newEmbed = new EmbedBuilder().setTitle(embed.title || 'Inventory').setColor(embed.color || 0x8B_45_13).setDescription(`💰 Total Value: ${inventoryValue} gold`);
  ...
  try {
    ...
    if (custom.startsWith('guess_submit:')) {
      ...
      if (isCorrect) {
        ...
        if (attemptsUsed === 1) performanceRating = '🌟 PERFECT! First try!';else if (attemptsUsed <= 3) performanceRating = '🥇 Excellent!';else if (attemptsUsed <= 5) performanceRating = '🥈 Good job!';else if (attemptsUsed <= 7) performanceRating = '🥉 Not bad!';else performanceRating = '🎯 You got it!';
        const embed = new EmbedBuilder().setTitle('🎉 Congratulations!').setColor(0x00_FF_00).setDescription(`You guessed **${gameState.secretNumber}** correctly!\n\n${performanceRating}`).addFields({
          name: '📊 Game Stats',
        ...
        if (currentAttemptsUsed >= attempts) {
          ...
          guessGames.delete(gameId); // Clean up completed game
          const timeElapsed = Math.round((Date.now() - gameState.startTime) / 1000);
          const loseEmbed = new EmbedBuilder().setTitle('❌ Game Over!').setColor(0xFF_00_00).setDescription(`The secret number was **${gameState.secretNumber}**!\n\nYou used all ${attempts} attempts in ${timeElapsed} seconds.`).addFields({
            name: 'Your Guesses',
        ...
        const embed = new EmbedBuilder().setTitle('🔢 Number Guessing Game').setColor(0x00_99_FF).setDescription(`I'm thinking of a number between **${min}** and **${max}**.\n\nYou have **${attempts - currentAttemptsUsed}** attempts remaining.\n\n**${guessNum}** - ${feedback}`).addFields({
          name: 'Previous Guesses',
          value: guesses.length > 0 ? guesses.slice(-5).map((/** @type {{number: number, feedback: string, attempt: number}} */g, /** @type {number} */i) => `**${g.number}** - ${g.feedback}`).join('\n') : 'No guesses yet',
          inline: false
        });
    ...
    await handleCommandError(interaction, error instanceof CommandError ? error : new CommandError('An error occurred while processing the modal.', 'UNKNOWN_ERROR', {
      originalError: error instanceof Error ? error.message : String(error)
    }));
  }
}
  ...
  try {
    ...
    if (action === 'explore_continue') {
      ...
      if (targetUserId && targetUserId !== interaction.user.id) {
        ...
        return safeInteractionReply(interaction, {
          content: 'You cannot continue adventure for another user.',
          flags: MessageFlags.Ephemeral
        });
      }
      const char = getCharacter(interaction.user.id);
      ...
      const embed = new EmbedBuilder().setTitle('🏃 Continue Adventure').setColor(0x21_96_F3).setDescription(result).addFields({
        name: '📊 Stats',
        value: `Level ${char.lvl} • XP ${char.xp} • Gold ${char.gold}`,
        inline: true
      });
      await safeInteractionUpdate(interaction, {
        embeds: [embed],
    ...
    if (action === 'explore_leave') {
      ...
      if (targetUserId && targetUserId !== interaction.user.id) {
        logCommandExecution(interaction, false, new Error('Wrong user'));
        return safeInteractionReply(interaction, {
          content: 'You cannot leave for another user.',
          flags: MessageFlags.Ephemeral
        });
      }
      const embed = new EmbedBuilder().setTitle('🏃 Leave Location').setColor(0xFF_98_00).setDescription(`You safely leave ${locationName} and return to town.\n\n*Your adventure continues another day!*`);
      await safeInteractionUpdate(interaction, {
        embeds: [embed],
        components: []
      });
    ...
    if (action === 'music_pause') {
      if (!interaction.guild) {
        return await safeInteractionReply(interaction, {
          content: '❌ **Music commands are only available in servers.**',
          flags: MessageFlags.Ephemeral
        });
      }
      ...
      if (targetGuild && targetGuild !== interaction.guild.id) {
        ...
        return await safeInteractionReply(interaction, {
          content: 'You cannot pause music in another server.',
          flags: MessageFlags.Ephemeral
        });
      }
    ...
    if (action === 'music_resume') {
      ...
      if (targetGuild && targetGuild !== interaction.guild.id) {
        logCommandExecution(interaction, false, new Error('Wrong guild'));
        return await safeInteractionReply(interaction, {
          content: 'You cannot resume music in another server.',
          flags: MessageFlags.Ephemeral
        });
      }
    ...
    if (action === 'music_skip') {
      ...
      if (targetGuild && targetGuild !== interaction.guild.id) {
        logCommandExecution(interaction, false, new Error('Wrong guild'));
        return await safeInteractionReply(interaction, {
          content: 'You cannot skip music in another server.',
          flags: MessageFlags.Ephemeral
        });
      }
      ...
      if (nextSong) {
        if (!interaction.message?.embeds?.[0]) {
          return await safeInteractionReply(interaction, {
            content: '❌ **Unable to update embed.**',
            flags: MessageFlags.Ephemeral
          });
        }
        const embed = EmbedBuilder.from(interaction.message.embeds[0]).setTitle('⏭️ Song Skipped').setDescription(`**Now Playing:** ${nextSong.title} by ${nextSong.artist}`).setColor(0xFF_A5_00);
        await safeInteractionUpdate(interaction, {
          embeds: [embed],
        ...
        await safeInteractionReply(interaction, {
          content: '❌ No songs in queue to skip.',
          flags: MessageFlags.Ephemeral
        });
      }
      return;
    ...
    if (action === 'music_stop') {
      ...
      if (targetGuild && targetGuild !== interaction.guild.id) {
        logCommandExecution(interaction, false, new Error('Wrong guild'));
        return await safeInteractionReply(interaction, {
          content: 'You cannot stop music in another server.',
          flags: MessageFlags.Ephemeral
        });
      }
      ...
      if (success) {
        ...
        const embed = EmbedBuilder.from(interaction.message.embeds[0]).setTitle('⏹️ Music Stopped').setDescription('Music stopped and left voice channel.').setColor(0xFF_00_00);
        await safeInteractionUpdate(interaction, {
          embeds: [embed],
          components: []
        });
    ...
    if (action === 'music_queue') {
      ...
      if (targetGuild && targetGuild !== interaction.guild.id) {
        logCommandExecution(interaction, false, new Error('Wrong guild'));
        return await safeInteractionReply(interaction, {
          content: 'You cannot view queue in another server.',
          flags: MessageFlags.Ephemeral
        });
      }
      ...
      const embed = new EmbedBuilder().setTitle('📋 Music Queue').setColor(0x00_99_FF).setDescription(description).addFields({
        name: '📊 Queue Info',
    ...
    if (action === 'music_shuffle') {
      ...
      if (targetGuild && targetGuild !== interaction.guild.id) {
        logCommandExecution(interaction, false, new Error('Wrong guild'));
        return await safeInteractionReply(interaction, {
          content: 'You cannot shuffle queue in another server.',
          flags: MessageFlags.Ephemeral
        });
      }
      ...
      if (success) {
        ...
        await safeInteractionReply(interaction, {
          content: '❌ Queue is empty or too small to shuffle.',
          flags: MessageFlags.Ephemeral
        });
      }
      return;
    ...
    if (action === 'music_clear') {
      ...
      if (targetGuild && targetGuild !== interaction.guild.id) {
        logCommandExecution(interaction, false, new Error('Wrong guild'));
        return await safeInteractionReply(interaction, {
          content: 'You cannot clear queue in another server.',
          flags: MessageFlags.Ephemeral
        });
      }
      const success = clearQueue(interaction.guild.id);
      if (success) {
        const embed = new EmbedBuilder().setTitle('🗑️ Queue Cleared').setColor(0xFF_45_00).setDescription('Music queue has been cleared!');
        await safeInteractionUpdate(interaction, {
          embeds: [embed],
          components: []
        });
    ...
    if (action === 'music_back') {
      ...
      if (targetGuild && targetGuild !== interaction.guild.id) {
        logCommandExecution(interaction, false, new Error('Wrong guild'));
        return await safeInteractionReply(interaction, {
          content: 'You cannot go back in another server.',
          flags: MessageFlags.Ephemeral
        });
      }
      const previousSong = back(interaction.guild.id);
      if (previousSong) {
        const embed = new EmbedBuilder().setTitle('⬅️ Back to Previous Song').setColor(0xFF_A5_00).setDescription(`**Now Playing:** ${previousSong.title} by ${previousSong.artist}`).setThumbnail(previousSong.thumbnail || 'https://i.imgur.com/SjIgjlE.png');
        await safeInteractionUpdate(interaction, {
          embeds: [embed],
    ...
    if (action === 'music_play') {
      ...
      if (!interaction.member) {
        return await safeInteractionReply(interaction, {
          content: '❌ **Unable to determine member information.**',
          flags: MessageFlags.Ephemeral
        });
      }
      ...
      if (!botMember) {
        return await safeInteractionReply(interaction, {
          content: '❌ **Bot member not found in guild.**',
          flags: MessageFlags.Ephemeral
        });
      }
      ...
      try {
        ...
        if (songs.length > songIndex) {
          ...
          if (!result.success) {
            ...
            const embed = new EmbedBuilder().setTitle('🎵 Now Playing').setColor(0x00_FF_00).setDescription(`**${song.title}** by **${song.artist}**`).addFields({
              name: '⏱️ Duration',
              value: song.duration,
              inline: true
            }, {
              name: '🔊 Volume',
              value: `${getMusicStats(interaction.guild.id).volume}%`,
              inline: true
            }, {
              name: '👤 Requested by',
            ...
            if (song.source === 'spotify') {
              embed.addFields({
                name: 'ℹ️ Note',
                value: 'Playing 30-second preview from Spotify',
                inline: false
              });
            } else if (song.source === 'youtube') {
              embed.addFields({
                name: 'ℹ️ Note',
                value: 'Playing full track from YouTube',
                inline: false
              });
            }
          ...
          await safeInteractionReply(interaction, {
            content: '❌ **Song no longer available**',
            flags: MessageFlags.Ephemeral
          });
        }
      } catch (error) {
    ...
    if (action === 'music_radio_change') {
      ...
      try {
        ...
        if (!voiceChannel) {
          logCommandExecution(interaction, false, new Error('No voice channel'));
          return await safeInteractionReply(interaction, {
            content: '🎵 You must be in a voice channel to change radio!',
            flags: MessageFlags.Ephemeral
          });
        }
        ...
        if (!result.success) {
          ...
          const embed = new EmbedBuilder().setTitle(`📻 Changed Station: ${station.name}`).setColor(0xFF_98_00).setDescription(`**${station.name}** radio is now playing!\n\n🎵 *Live streaming activated*`).addFields({
            name: '📻 Station',
            value: station.name,
            inline: true
          }, {
    ...
    if (action === 'explore_unlock') {
      ...
      if (targetUserId && targetUserId !== interaction.user.id) {
        logCommandExecution(interaction, false, new Error('Wrong user'));
        return safeInteractionReply(interaction, {
          content: 'You cannot unlock locations for another user.',
          flags: MessageFlags.Ephemeral
        });
      }
      const char = getCharacter(interaction.user.id);
      ...
      if (!nextLocationId) {
        ...
        const updatedEmbed = EmbedBuilder.from(currentEmbed).setDescription('🏕️ **No new locations available!**\n\nYou\'ve discovered all currently available locations. Check back later for new content!');
        ...
        if (unlockResult.success) {
          ...
          // Create success embed
          const successEmbed = new EmbedBuilder().setTitle('🎉 New Location Discovered!').setColor(unlockedLocation.color).setDescription(unlockResult.message || 'New location unlocked!').addFields({
            name: '📍 Location',
            value: unlockedLocation.name,
            inline: true
          }, {
            name: '🏆 Level',
            value: unlockedLocation.level,
            inline: true
          }, {
            name: '🎯 Type',
            value: unlockedLocation.type,
            inline: true
          }, {
            name: '💎 Rewards',
            value: `${unlockedLocation.rewards.xp} XP, ${unlockedLocation.rewards.gold} gold`,
            inline: false
          });
          ...
          // Unlock failed
          const errorEmbed = new EmbedBuilder().setTitle('❌ Location Unlock Failed').setColor(0xFF_00_00).setDescription(`Failed to unlock location: ${unlockResult.reason || 'Unknown error'}`);
          await safeInteractionReply(interaction, {
            embeds: [errorEmbed],
            flags: MessageFlags.Ephemeral
          });
      ...
      if (targetUserId && targetUserId !== interaction.user.id) {
        logCommandExecution(interaction, false, new Error('Wrong user'));
        return safeInteractionReply(interaction, {
          content: 'You cannot view map for another user.',
          flags: MessageFlags.Ephemeral
        });
      }
      const char = getCharacter(interaction.user.id);
      ...
      const mapEmbed = new EmbedBuilder().setTitle('🗺️ World Map').setColor(0x00_99_FF).setDescription(mapText).setFooter({
        text: 'Use /explore locations to view details and rewards for each location'
      });
    ...
    if (action === 'rpg_leaderboard') {
      ...
      if (targetUserId && targetUserId !== interaction.user.id) {
        logCommandExecution(interaction, false, new Error('Wrong user'));
        return safeInteractionReply(interaction, {
          content: 'You cannot view another user\'s leaderboard.',
          flags: MessageFlags.Ephemeral
        });
      }
      const limit = 10;
      ...
      const embed = new EmbedBuilder().setTitle('🏆 RPG Leaderboard').setColor(0xFF_D7_00).setDescription(`Leaderboard — Page ${page}/${totalPages}\n\n${list}`);
    ...
    if (action === 'rpg_reset_modal') {
      ...
      if (targetUserId && targetUserId !== interaction.user.id) {
        logCommandExecution(interaction, false, new Error('Wrong user'));
        return safeInteractionReply(interaction, {
          content: 'You cannot reset another user\'s character.',
          flags: MessageFlags.Ephemeral
        });
      }
    ...
    if (action === 'explore_investigate') {
      ...
      if (targetUserId && targetUserId !== interaction.user.id) {
        logCommandExecution(interaction, false, new Error('Wrong user'));
        return safeInteractionReply(interaction, {
          content: 'You cannot explore for another user.',
          flags: MessageFlags.Ephemeral
        });
      }
      const char = getCharacter(interaction.user.id);
      ...
      applyXp(interaction.user.id, char, xpGain);
      saveCharacter(interaction.user.id, char);
      const embed = new EmbedBuilder().setTitle('🔍 Investigation Results').setColor(0x4C_AF_50).setDescription(result).addFields({
        name: '📊 Stats',
        value: `Level ${char.lvl} • XP ${char.xp} • Gold ${char.gold}`,
        inline: true
      });
    ...
    if (action === 'explore_search') {
      ...
      applyXp(interaction.user.id, char, xpGain);
      saveCharacter(interaction.user.id, char);
      const embed = new EmbedBuilder().setTitle('⚔️ Search Results').setColor(event === 'trap' ? 0xFF_00_00 : 0x21_96_F3).setDescription(result).addFields({
        name: '📊 Stats',
        value: `Level ${char.lvl} • XP ${char.xp} • Gold ${char.gold}`,
        inline: true
      });
    ...
    if (action === 'explore_rest') {
      ...
      if (targetUserId && targetUserId !== interaction.user.id) {
        logCommandExecution(interaction, false, new Error('Wrong user'));
        return safeInteractionReply(interaction, {
          content: 'You cannot rest for another user.',
          flags: MessageFlags.Ephemeral
        });
      }
      const char = getCharacter(interaction.user.id);
      ...
      try {
        ...
        saveCharacter(interaction.user.id, char);
        const embed = new EmbedBuilder().setTitle('🛌 Rest Results').setColor(0x4C_AF_50).setDescription(`You take a peaceful rest in the safety of ${locationId}.\n❤️ HP +${hpGain} → ${char.hp}/${char.maxHp}\n🔵 MP +${mpGain} → ${char.mp}/${char.maxMp}`).addFields({
          name: '📊 Stats',
          value: `Level ${char.lvl} • XP ${char.xp} • Gold ${char.gold}`,
          inline: true
        });
        ...
        return safeInteractionReply(interaction, {
          content: '❌ **Failed to rest!** Please try again later.',
          flags: MessageFlags.Ephemeral
        });
      }
      if (action === 'explore_continue') {
      ...
      if (action === 'economy_transfer') {
        ...
        if (targetUserId && targetUserId !== interaction.user.id) {
          logCommandExecution(interaction, false, new Error('Wrong user'));
          await safeInteractionReply(interaction, {
            content: 'You cannot transfer for another user.',
            flags: MessageFlags.Ephemeral
          });
          return;
        }
      ...
      if (action === 'economy_market') {
        ...
        if (targetUserId && targetUserId !== interaction.user.id) {
          logCommandExecution(interaction, false, new Error('Wrong user'));
          await safeInteractionReply(interaction, {
            content: 'You cannot access market for another user.',
            flags: MessageFlags.Ephemeral
          });
          return;
        }
        const marketPrices = getMarketPrice();
        const embed = new EmbedBuilder().setTitle('🛒 Market Prices').setColor(0x4C_AF_50).setDescription('Current market prices:');
        ...
        await safeInteractionUpdate(interaction, {
          embeds: [embed],
          components: [row.map(r => r.toJSON ? r.toJSON() : r)]
        });
        return;
      }
      if (action === 'economy_business') {
        ...
        if (targetUserId && targetUserId !== interaction.user.id) {
          logCommandExecution(interaction, false, new Error('Wrong user'));
          await safeInteractionReply(interaction, {
            content: 'You cannot manage business for another user.',
            flags: MessageFlags.Ephemeral
          });
          return;
        }
        ...
        const embed = new EmbedBuilder().setTitle('🏪 Business Income').setColor(0x4C_AF_50).setDescription(`Your business generated **${income}** gold today!\n💰 New balance: **${balanceUpdate}** gold`);
        await safeInteractionUpdate(interaction, {
          embeds: [embed],
          components: []
        });
      ...
      if (action === 'economy_invest') {
        ...
        if (targetUserId && targetUserId !== interaction.user.id) {
          logCommandExecution(interaction, false, new Error('Wrong user'));
          await safeInteractionReply(interaction, {
            content: 'You cannot invest for another user.',
            flags: MessageFlags.Ephemeral
          });
          return;
        }
        ...
        if (currentBalance < 100) {
          await safeInteractionReply(interaction, {
            content: '❌ You need at least 100 gold to invest.',
            flags: MessageFlags.Ephemeral
          });
          return;
        }
        ...
        const embed = new EmbedBuilder().setTitle('📈 Investment Results').setColor(profit > 0 ? 0x4C_AF_50 : 0xFF_00_00).setDescription(`You invested **${investment}** gold.\n${profit > 0 ? '📈 Profit' : '📉 Loss'}: **${Math.abs(profit)}** gold\n💰 New balance: **${getBalance(interaction.user.id)}** gold`);
        await safeInteractionUpdate(interaction, {
          embeds: [embed],
          components: []
        });
      ...
      if (action === 'economy_buy') {
        ...
        if (targetUserId && targetUserId !== interaction.user.id) {
          logCommandExecution(interaction, false, new Error('Wrong user'));
          await safeInteractionReply(interaction, {
            content: 'You cannot buy for another user.',
            flags: MessageFlags.Ephemeral
          });
          return;
        }
        const marketPrices = getMarketPrice();
        const embed = new EmbedBuilder().setTitle('🛒 Buy from Market').setColor(0x21_96_F3).setDescription('Select an item to buy:');
        let description = '';
        for (const [item, price] of Object.entries(marketPrices)) {
      ...
      if (action === 'economy_sell') {
        ...
        if (targetUserId && targetUserId !== interaction.user.id) {
          logCommandExecution(interaction, false, new Error('Wrong user'));
          await safeInteractionReply(interaction, {
            content: 'You cannot sell for another user.',
            flags: MessageFlags.Ephemeral
          });
          return;
        }
        ...
        const embed = new EmbedBuilder().setTitle('💸 Sell Items').setColor(0xFF_98_00).setDescription('Select items to sell from your inventory:');
        let description = '';
      ...
      if (action === 'trade_create_auction') {
        ...
        if (targetUserId && targetUserId !== interaction.user.id) {
          logCommandExecution(interaction, false, new Error('Wrong user'));
          return safeInteractionReply(interaction, {
            content: 'You cannot create auctions for another user.',
            flags: MessageFlags.Ephemeral
          });
        }
      ...
      if (action === 'trade_view_auctions') {
        ...
        if (targetUserId && targetUserId !== interaction.user.id) {
          logCommandExecution(interaction, false, new Error('Wrong user'));
          return safeInteractionReply(interaction, {
            content: 'You cannot view auctions for another user.',
            flags: MessageFlags.Ephemeral
          });
        }
        const auctions = getActiveAuctions();
        const embed = new EmbedBuilder().setTitle('🔍 Active Auctions').setColor(0x21_96_F3).setDescription(auctions.length > 0 ? auctions.slice(0, 10).map(a => `• ${a.itemName} x${a.quantity} - Starting: ${a.startingPrice} gold - Seller: ${a.seller}`).join('\n') : 'No active auctions at the moment.');
        await safeInteractionUpdate(interaction, {
          embeds: [embed],
          components: []
        });
      ...
      if (action === 'profile_edit') {
        ...
        if (targetUserId && targetUserId !== interaction.user.id) {
          logCommandExecution(interaction, false, new Error('Wrong user'));
          return safeInteractionReply(interaction, {
            content: 'You cannot edit another user\'s profile.',
            flags: MessageFlags.Ephemeral
          });
        }
      ...
      if (action === 'profile_refresh') {
        ...
        if (targetUserId && targetUserId !== interaction.user.id) {
          logCommandExecution(interaction, false, new Error('Wrong user'));
          return safeInteractionReply(interaction, {
            content: 'You cannot refresh another user\'s profile.',
            flags: MessageFlags.Ephemeral
          });
        }
        const profile = updateProfile(interaction.user.id, {});
        const embed = new EmbedBuilder().setTitle('🔄 Profile Refreshed').setColor(0x4C_AF_50).setDescription('Profile data has been refreshed!');
        await safeInteractionUpdate(interaction, {
          embeds: [embed],
          components: interaction.message.components
        });
      ...
      if (action === 'profile_compare') {
        ...
        if (targetUserId && targetUserId !== interaction.user.id) {
          logCommandExecution(interaction, false, new Error('Wrong user'));
          return safeInteractionReply(interaction, {
            content: 'You cannot compare profiles for another user.',
            flags: MessageFlags.Ephemeral
          });
        }
        ...
        if (!interaction.guild) {
          return await safeInteractionReply(interaction, {
            content: '❌ **Profile comparison is only available in servers.**',
            flags: MessageFlags.Ephemeral
          });
        }
        if (!compareUserId) {
          return await safeInteractionReply(interaction, {
            content: '❌ **Invalid user ID for comparison.**',
            flags: MessageFlags.Ephemeral
          });
        }
        ...
        if (!compareUser) {
          return safeInteractionReply(interaction, {
            content: '❌ Could not find the user to compare with.',
            flags: MessageFlags.Ephemeral
          });
        }

        // Create comparison embed
        const embed = new EmbedBuilder().setTitle('⚖️ Profile Comparison').setColor(0x21_96_F3).setDescription(`Comparing **${interaction.user.username}** vs **${compareUser.username}**`);
      ...
      if (action === 'remind_upcoming') {
        ...
        if (targetUserId && targetUserId !== interaction.user.id) {
          logCommandExecution(interaction, false, new Error('Wrong user'));
          return safeInteractionReply(interaction, {
            content: 'You cannot view reminders for another user.',
            flags: MessageFlags.Ephemeral
          });
        }
        const embed = new EmbedBuilder().setTitle('📅 Upcoming Reminders').setColor(0xFF_98_00).setDescription('No upcoming reminders set.');
        await safeInteractionUpdate(interaction, {
          embeds: [embed],
          components: []
        });
      ...
      if (action.startsWith('memory_')) {
        ...
        if (cardIndexStr === 'reset') {
          ...
          if (targetUserId && targetUserId !== interaction.user.id) {
            logCommandExecution(interaction, false, new Error('Wrong user'));
            return safeInteractionReply(interaction, {
              content: 'You cannot reset memory for another user.',
              flags: MessageFlags.Ephemeral
            });
          }
        ...
        if (!gameState.gameActive) {
          return safeInteractionReply(interaction, {
            content: '❌ **Game is already completed!**',
            flags: MessageFlags.Ephemeral
          });
        }
        ...
        if (card.isMatched) {
          return safeInteractionReply(interaction, {
            content: '❌ **Card is already matched!**',
            flags: MessageFlags.Ephemeral
          });
        }
        if (gameState.flippedCards.includes(cardIndex)) {
          return safeInteractionReply(interaction, {
            content: '❌ **Card is already flipped!**',
            flags: MessageFlags.Ephemeral
          });
        }
        ...
        if (gameState.flippedCards.length === 2) {
          ...
          if (firstCard.emoji === secondCard.emoji) {
            ...
            if (gameState.matchedPairs === gameState.totalPairs) {
              gameState.gameActive = false;
              const timeElapsed = Math.round((Date.now() - gameState.startTime) / 1000);
              const winEmbed = new EmbedBuilder().setTitle('🎉 Memory Master!').setDescription(`Congratulations! You matched all ${gameState.totalPairs} pairs in ${gameState.moves} moves and ${timeElapsed} seconds! 🏆`).setColor(0x00_FF_00).addFields({
                name: '📊 Stats',
      ...
      if (action === 'inventory_refresh') {
        ...
        if (targetUserId && targetUserId !== interaction.user.id) {
          logCommandExecution(interaction, false, new Error('Wrong user'));
          await safeInteractionReply(interaction, {
            content: 'You cannot refresh inventory for another user.',
            flags: MessageFlags.Ephemeral
          });
          return;
        }
        ...
        const embed = new EmbedBuilder().setTitle('🎒 Inventory').setColor(0x8B_45_13).setDescription(`💰 Total Value: ${inventoryValue} gold`);
      ...
      if (action === 'inventory_random') {
        ...
        if (targetUserId && targetUserId !== interaction.user.id) {
          logCommandExecution(interaction, false, new Error('Wrong user'));
          return safeInteractionReply(interaction, {
            content: 'You cannot get random items for another user.',
            flags: MessageFlags.Ephemeral
          });
        }
        const char = getCharacter(interaction.user.id);
        ...
        const embed = new EmbedBuilder().setTitle('🎲 Random Item').setColor(0xFF_D7_00).setDescription(`You found a **${randomItem.name}**!\n\n${randomItem.description}`).addFields({
          name: '📊 Stats',
          value: `Rarity: ${randomItem.rarity} • Value: ${randomItem.value} gold`,
          inline: true
        });
        await safeInteractionUpdate(interaction, {
          embeds: [embed],
      ...
      if (action === 'inventory_sell_all') {
        ...
        if (targetUserId && targetUserId !== interaction.user.id) {
          logCommandExecution(interaction, false, new Error('Wrong user'));
          return safeInteractionReply(interaction, {
            content: 'You cannot sell items for another user.',
            flags: MessageFlags.Ephemeral
          });
        }
        const char = getCharacter(interaction.user.id);
        ...
        for (const [itemId, quantity] of Object.entries(inventory)) {
          ...
          if (item && item.rarity === 'common') {
            // Only sell common items as "junk"
        ...
        char.gold += totalGold;
        saveCharacter(interaction.user.id, char);
        const embed = new EmbedBuilder().setTitle('💰 Sold Junk Items').setColor(0x4C_AF_50).setDescription(`Sold ${itemsSold} common items for ${totalGold} gold!\n💰 New balance: ${char.gold} gold`);
        await safeInteractionUpdate(interaction, {
          embeds: [embed],
          components: []
        });
      ...
      if (action === 'guild_contribute') {
        ...
        if (targetUserId && targetUserId !== interaction.user.id) {
          logCommandExecution(interaction, false, new Error('Wrong user'));
          return safeInteractionReply(interaction, {
            content: 'You cannot contribute to guild for another user.',
            flags: MessageFlags.Ephemeral
          });
        }
      ...
      if (action === 'guild_refresh') {
        ...
        if (targetUserId && targetUserId !== interaction.user.id) {
          logCommandExecution(interaction, false, new Error('Wrong user'));
          return safeInteractionReply(interaction, {
            content: 'You cannot refresh guild for another user.',
            flags: MessageFlags.Ephemeral
          });
        }
        const guildInfo = getUserGuild(interaction.user.id);
        const embed = new EmbedBuilder().setTitle('🔄 Guild Refreshed').setColor(0x4C_AF_50).setDescription(`${guildName || guildInfo?.name || 'Guild'} data has been refreshed!`);
        await safeInteractionUpdate(interaction, {
          embeds: [embed],
          components: interaction.message.components
        });
      ...
      if (action === 'party_invite') {
        ...
        if (targetUserId && targetUserId !== interaction.user.id) {
          logCommandExecution(interaction, false, new Error('Wrong user'));
          return safeInteractionReply(interaction, {
            content: 'You cannot generate invites for another user.',
            flags: MessageFlags.Ephemeral
          });
        }
        const embed = new EmbedBuilder().setTitle('🔗 Party Invite Generated').setColor(0x21_96_F3).setDescription(`Invite link for party ${partyId}: \`/join ${partyId}\``);
        await safeInteractionUpdate(interaction, {
          embeds: [embed],
          components: []
        });
      ...
      if (action === 'fun_joke') {
        ...
        if (targetUserId && targetUserId !== interaction.user.id) {
          logCommandExecution(interaction, false, new Error('Wrong user'));
          await safeInteractionReply(interaction, {
            content: 'You cannot get jokes for another user.',
            flags: MessageFlags.Ephemeral
          });
          return;
        }
        const joke = getRandomJoke(category);
        const embed = new EmbedBuilder().setTitle('😂 Joke').setColor(0xFF_D7_00).setDescription(String(joke || 'No joke available.'));
      ...
      if (action === 'fun_rate') {
        ...
        if (targetUserId && targetUserId !== interaction.user.id) {
          logCommandExecution(interaction, false, new Error('Wrong user'));
          await safeInteractionReply(interaction, {
            content: 'You cannot rate jokes for another user.',
            flags: MessageFlags.Ephemeral
          });
          return;
        }
        ...
        if (isNaN(rating) || rating < 1 || rating > 5) {
          await safeInteractionReply(interaction, {
            content: '❌ Invalid rating. Rating must be between 1 and 5 stars.',
            flags: MessageFlags.Ephemeral
          });
          return;
        }
        ...
        if (!jokeId || jokeId.length < 10) {
          await safeInteractionReply(interaction, {
            content: '❌ Invalid joke reference.',
            flags: MessageFlags.Ephemeral
          });
          return;
        }
        try {
          ...
          if (ratingResult !== true) {
            return safeInteractionReply(interaction, {
              content: '❌ Failed to record your rating. Please try again.',
              flags: MessageFlags.Ephemeral
            });
          }

          // Create success response embed
          const embed = new EmbedBuilder().setTitle('⭐ Thanks for your rating!').setColor(0x4C_AF_50).setDescription(`You rated this joke with **${rating} star${rating !== 1 ? 's' : ''}!** ⭐\n\nYour feedback helps improve our joke collection!`).setFooter({
          ...
          return safeInteractionReply(interaction, {
            content: '❌ An error occurred while recording your rating. Please try again.',
            flags: MessageFlags.Ephemeral
          });
        }
        return;
      ...
      if (action === 'fun_story') {
        ...
        if (targetUserId && targetUserId !== interaction.user.id) {
          logCommandExecution(interaction, false, new Error('Wrong user'));
          await safeInteractionReply(interaction, {
            content: 'You cannot get stories for another user.',
            flags: MessageFlags.Ephemeral
          });
          return;
        }
        const story = generateStory(genre);
        const embed = new EmbedBuilder().setTitle('📖 Story').setColor(0x99_32_CC).setDescription(String(story || 'No story available.'));
      ...
      if (action === 'fun_share') {
        ...
        if (targetUserId && targetUserId !== interaction.user.id) {
          logCommandExecution(interaction, false, new Error('Wrong user'));
          await safeInteractionReply(interaction, {
            content: 'You cannot share for another user.',
            flags: MessageFlags.Ephemeral
          });
          return;
        }
        ...
        try {
          switch (content.type) {
            case 'story':
              {
                shareEmbed = new EmbedBuilder().setTitle(`📖 ${content.genre.charAt(0).toUpperCase() + content.genre.slice(1)} Story`).setColor(0x99_32_CC).setDescription(content.story).addFields({
                  name: '🎯 Prompt',
            ...
            case 'fact':
              {
                shareEmbed = new EmbedBuilder().setTitle(`🧠 ${content.category === 'random' ? 'Random' : content.category.charAt(0).toUpperCase() + content.category.slice(1)} Fun Fact`).setColor(0x4C_AF_50).setDescription(content.fact).setFooter({
            ...
            case 'quote':
              {
                shareEmbed = new EmbedBuilder().setTitle(`💬 ${content.category.charAt(0).toUpperCase() + content.category.slice(1)} Quote`).setColor(0xE9_1E_63).addFields({
                  name: 'Quote',
                  value: `"${content.quote}"`,
                  inline: false
                }, {
          ...
          if (!interaction.channel) {
            return await safeInteractionUpdate(interaction, {
              embeds: [errorEmbed],
              components: []
            });
          }
          ...
          const successEmbed = new EmbedBuilder().setTitle('✅ Content Shared Successfully!').setColor(0x4C_AF_50).setDescription('Your content has been shared with the channel!').setFooter({
            text: 'Thanks for spreading the fun!'
          });
          await safeInteractionUpdate(interaction, {
            embeds: [successEmbed],
            components: []
          ...
          const errorEmbed = new EmbedBuilder().setTitle('❌ Sharing Failed').setColor(0xFF_00_00).setDescription('Sorry, there was an error sharing your content. Please try again.').setFooter({
            text: 'If this persists, contact the bot administrator'
          });
          await safeInteractionUpdate(interaction, {
            embeds: [errorEmbed],
            components: []
      ...
      if (action === 'fun_riddle') {
        ...
        if (targetUserId && targetUserId !== interaction.user.id) {
          logCommandExecution(interaction, false, new Error('Wrong user'));
          await safeInteractionReply(interaction, {
            content: 'You cannot view riddles for another user.',
            flags: MessageFlags.Ephemeral
          });
          return;
        }
        const riddle = getRiddle(difficulty);
        const embed = new EmbedBuilder().setTitle('💡 Riddle').setColor(0xFF_98_00).setDescription(`**${riddle.question}**\n\n${riddle.hint ? `*Hint: ${riddle.hint}*` : ''}`);
      ...
      if (action === 'fun_riddle_new') {
        ...
        if (targetUserId && targetUserId !== interaction.user.id) {
          logCommandExecution(interaction, false, new Error('Wrong user'));
          await safeInteractionReply(interaction, {
            content: 'You cannot get new riddles for another user.',
            flags: MessageFlags.Ephemeral
          });
          return;
        }
        const riddle = getRiddle(difficulty);
        const embed = new EmbedBuilder().setTitle('🧩 New Riddle').setColor(0xFF_98_00).setDescription(`**${riddle.question}**\n\n${riddle.hint ? `*Hint: ${riddle.hint}*` : ''}`);
      ...
      if (action === 'fun_fact') {
        ...
        if (targetUserId && targetUserId !== interaction.user.id) {
          logCommandExecution(interaction, false, new Error('Wrong user'));
          await safeInteractionReply(interaction, {
            content: 'You cannot get facts for another user.',
            flags: MessageFlags.Ephemeral
          });
          return;
        }
        ...
        const embed = new EmbedBuilder().setTitle('🧠 Fun Fact').setColor(0x4C_AF_50).setDescription(`${factText}`);
      ...
      if (action === 'fun_quote') {
        ...
        if (targetUserId && targetUserId !== interaction.user.id) {
          logCommandExecution(interaction, false, new Error('Wrong user'));
          await safeInteractionReply(interaction, {
            content: 'You cannot get quotes for another user.',
            flags: MessageFlags.Ephemeral
          });
          return;
        }
        const quote = getRandomQuote(category);
        const embed = new EmbedBuilder().setTitle('💬 Quote').setColor(0x99_32_CC).setDescription(`"${quote.text || 'No quote text'}"\n\n— ${quote.author || 'Unknown'}`);
      ...
      if (action === 'fun_8ball') {
        ...
        if (targetUserId && targetUserId !== interaction.user.id) {
          logCommandExecution(interaction, false, new Error('Wrong user'));
          await safeInteractionReply(interaction, {
            content: 'You cannot use 8ball for another user.',
            flags: MessageFlags.Ephemeral
          });
          return;
        }
        const answer = magic8Ball();
        const embed = new EmbedBuilder().setTitle('🔮 Magic 8-Ball').setColor(0x00_00_00).setDescription(`🎱 ${answer}`);
      ...
      if (action === 'fun_name') {
        ...
        if (targetUserId && targetUserId !== interaction.user.id) {
          logCommandExecution(interaction, false, new Error('Wrong user'));
          await safeInteractionReply(interaction, {
            content: 'You cannot generate names for another user.',
            flags: MessageFlags.Ephemeral
          });
          return;
        }
        const name = generateFunName(type);
        const embed = new EmbedBuilder().setTitle('🎭 Fun Name').setColor(0xFF_69_B4).setDescription(`**${String(name || 'No name available')}**`);
      ...
      if (action === 'fun_name_random') {
        ...
        const embed = new EmbedBuilder().setTitle('🎲 Random Fun Name').setColor(0xFF_69_B4).setDescription(`**${name}** (${randomType})`);
      ...
      if (action === 'fun_challenge') {
        ...
        if (targetUserId && targetUserId !== interaction.user.id) {
          logCommandExecution(interaction, false, new Error('Wrong user'));
          await safeInteractionReply(interaction, {
            content: 'You cannot get challenges for another user.',
            flags: MessageFlags.Ephemeral
          });
          return;
        }
        const challenge = createFunChallenge(type);
        const embed = new EmbedBuilder().setTitle('🎯 Challenge').setColor(0xFF_45_00).setDescription(String(challenge || 'No challenge available.'));
      ...
      if (action === 'fun_challenge_new') {
        ...
        const challenge = createFunChallenge(type);
        const embed = new EmbedBuilder().setTitle('🔄 New Challenge').setColor(0xFF_45_00).setDescription(String(challenge || 'No challenge available.'));
      ...
      if (action === 'ai_chat') {
        ...
        if (targetUserId && targetUserId !== interaction.user.id) {
          logCommandExecution(interaction, false, new Error('Wrong user'));
          await safeInteractionReply(interaction, {
            content: 'You cannot continue chat for another user.',
            flags: MessageFlags.Ephemeral
          });
          return;
        }
      ...
      if (action === 'ai_clear') {
        ...
        if (targetUserId && targetUserId !== interaction.user.id) {
          logCommandExecution(interaction, false, new Error('Wrong user'));
          await safeInteractionReply(interaction, {
            content: 'You cannot clear history for another user.',
            flags: MessageFlags.Ephemeral
          });
          return;
        }
        const embed = new EmbedBuilder().setTitle('🗑️ AI History Cleared').setColor(0xFF_00_00).setDescription('Chat history has been cleared!');
        await safeInteractionUpdate(interaction, {
          embeds: [embed],
          components: []
        });
      ...
      if (action === 'admin_warn') {
        ...
        const embed = new EmbedBuilder().setTitle('⚠️ User Warned').setColor(0xFF_A5_00).setDescription(`User <@${targetUserId}> has been warned.`);
        await safeInteractionUpdate(interaction, {
          embeds: [embed],
          components: []
        });
      ...
      if (action === 'admin_mute') {
        if (!interaction.guild) {
          return await safeInteractionReply(interaction, {
            content: '❌ **Admin commands are only available in servers.**',
            flags: MessageFlags.Ephemeral
          });
        }
        const [, targetUserId, guildId] = interaction.customId.split(':');
        ...
        try {
          ...
          const embed = new EmbedBuilder().setTitle('🔇 User Muted').setColor(0xFF_00_00).setDescription(`User <@${targetUserId}> has been muted.`);
          await safeInteractionUpdate(interaction, {
            embeds: [embed],
            components: []
          });
      ...
      if (action === 'admin_unmute') {
        ...
        try {
          ...
          const embed = new EmbedBuilder().setTitle('🔊 User Unmuted').setColor(0x4C_AF_50).setDescription(`User <@${targetUserId}> has been unmuted.`);
          await safeInteractionUpdate(interaction, {
            embeds: [embed],
            components: []
          });
      ...
      if (action === 'admin_unban') {
        ...
        try {
          ...
          const embed = new EmbedBuilder().setTitle('✅ User Unbanned').setColor(0x4C_AF_50).setDescription(`User <@${targetUserId}> has been unbanned.`);
          await safeInteractionUpdate(interaction, {
            embeds: [embed],
            components: []
          });
      ...
      if (action === 'achievements_refresh') {
        ...
        if (targetUserId && targetUserId !== interaction.user.id) {
          logCommandExecution(interaction, false, new Error('Wrong user'));
          return safeInteractionReply(interaction, {
            content: 'You cannot refresh achievements for another user.',
            flags: MessageFlags.Ephemeral
          });
        }
        const embed = new EmbedBuilder().setTitle('🔄 Achievements Refreshed').setColor(0x4C_AF_50).setDescription('Achievement data has been refreshed!');
        await safeInteractionUpdate(interaction, {
          embeds: [embed],
          components: interaction.message.components
        });
      ...
      if (action === 'achievements_leaderboard') {
        ...
        if (targetUserId && targetUserId !== interaction.user.id) {
          logCommandExecution(interaction, false, new Error('Wrong user'));
          return safeInteractionReply(interaction, {
            content: 'You cannot view leaderboard for another user.',
            flags: MessageFlags.Ephemeral
          });
        }
        ...
        if (!leaderboard || leaderboard.length === 0) {
          const embed = new EmbedBuilder().setTitle('🏅 Achievement Leaderboard').setColor(0xFF_D7_00).setDescription('No achievement data available yet.\n\nComplete achievements to appear on the leaderboard!');
          await safeInteractionUpdate(interaction, {
            embeds: [embed],
            components: []
          });
        ...
        const embed = new EmbedBuilder().setTitle('🏅 Achievement Leaderboard').setColor(0xFF_D7_00).setDescription(description).setFooter({
          text: 'Top achievers this month'
        });
        await safeInteractionUpdate(interaction, {
          embeds: [embed],
          components: []
      ...
      if (action === 'wordle_guess') {
        ...
        if (targetUserId && targetUserId !== interaction.user.id) {
          logCommandExecution(interaction, false, new Error('Wrong user'));
          return safeInteractionReply(interaction, {
            content: 'You cannot guess for another user.',
            flags: MessageFlags.Ephemeral
          });
        }
      ...
      if (action === 'ttt') {
        ...
        if (winner) {
          ...
          if (gameState.players.X.id !== 'ai') {
            await updateUserStats(gameState.players.X.id, {
              games: {
                tictactoe_games: 1
              }
            });
          ...
          if (gameState.players.O.id !== 'ai') {
            await updateUserStats(gameState.players.O.id, {
              games: {
                tictactoe_games: 1
              }
            });
          }
          const resultEmbed = new EmbedBuilder().setTitle('⭕ Tic-Tac-Toe - Game Over!').setColor(winner === 'tie' ? 0xFF_A5_00 : 0x00_FF_00).setDescription(winner === 'tie' ? '🤝 **It\'s a tie!**' : `🎉 **${gameState.players[winner].name} wins!**`).addFields({
            name: 'Final Board',
        ...
        if (gameState.isAI && gameState.currentPlayer === 'O' && gameState.status === 'active') {
          ...
          const {
            getAIMove
          } = await import('./commands/tictactoe.js');
          ...
          if (aiMove !== null) {
            ...
            if (aiWinner) {
              ...
              const resultEmbed = new EmbedBuilder().setTitle('⭕ Tic-Tac-Toe - Game Over!').setColor(aiWinner === 'tie' ? 0xFF_A5_00 : 0x00_FF_00).setDescription(aiWinner === 'tie' ? '🤝 **It\'s a tie!**' : `🎉 **${gameState.players[aiWinner].name} wins!**`).addFields({
                name: 'Final Board',
                value: formatBoard(gameState.board),
                inline: false
              });
      ...
      if (action === 'poll') {
        ...
        const updatedEmbed = new EmbedBuilder().setTitle(`📊 ${pollData.question}`).setColor(0x00_99_FF).setDescription(pollData.options.map((/** @type {string} */option, /** @type {number} */index) => {
          ...
          text: `Total votes: ${pollData.totalVotes} • Poll ends`
        }).setTimestamp(pollData.endTime);

        // Create updated button rows
        const buttons = pollData.options.map((option, index) => new ButtonBuilder().setCustomId(`poll_${index}`).setLabel(`${emojis[index]} ${option.length > 15 ? option.slice(0, 15) + '...' : option}`).setStyle(ButtonStyle.Primary));
        const rows = [];
      ...
      if (action === 'trivia') {
        ...
        // Send feedback
        const feedbackEmbed = new EmbedBuilder().setTitle(isCorrect ? '✅ Correct!' : '❌ Incorrect!').setDescription(`**${currentQuestion.question}**\n\nYour answer: **${currentQuestion.options[selectedAnswer]}**\nCorrect answer: **${currentQuestion.options[currentQuestion.correct]}**`).setColor(isCorrect ? 0x00_FF_00 : 0xFF_00_00).setFooter({
          text: `Score: ${gameState.score}/${gameState.currentQuestion}`
        });
        await safeInteractionReply(interaction, {
        ...
        if (gameState.currentQuestion >= gameState.questions.length) {
          ...
          let resultMessage = '';
          if (percentage >= 90) resultMessage = '🏆 Outstanding! You\'re a trivia master!';else if (percentage >= 70) resultMessage = '🥇 Great job! You know your stuff!';else if (percentage >= 50) resultMessage = '🥈 Not bad! Keep practicing!';else resultMessage = '📚 Keep learning and try again!';
          ...
          const resultEmbed = new EmbedBuilder().setTitle('🎯 Trivia Quiz Complete!').setDescription(`${resultMessage}\n\n**Final Score: ${gameState.score}/${gameState.questions.length} (${percentage}%)**\n⏱️ Time: ${totalTime}s`).setColor(percentage >= 70 ? 0x00_FF_00 : percentage >= 50 ? 0xFF_A5_00 : 0xFF_00_00).setTimestamp();
          ...
          setTimeout(async () => {
            ...
            const embed = new EmbedBuilder().setTitle(`🧠 Trivia Quiz - Question ${gameState.currentQuestion + 1}/${gameState.questions.length}`).setDescription(`**${nextQuestion.question}**`).setColor(0x00_99_FF).addFields({
              name: 'Category',
            ...
            const buttons = nextQuestion.options.map((/** @type {string} */option, /** @type {number} */index) => new ButtonBuilder().setCustomId(`trivia_${index}`).setLabel(`${String.fromCharCode(65 + index)}) ${option}`).setStyle(ButtonStyle.Primary));
            const rows = [];
            for (let i = 0; i < buttons.length; i += 2) {
              const row = new ActionRowBuilder().addComponents(buttons.slice(i, i + 2));
              rows.push(row);
