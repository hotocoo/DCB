...
export async function safeInteractionReply(interaction, options) {
  ...
  try {
    ...
    logger.debug(`Attempting to reply to interaction ${interactionId}`);
...
async function handleModalSubmit(interaction, client) {
  ...
  try {
    ...
    if (custom.startsWith('rpg_reset_confirm:')) {
      ...
      logger.debug(`Modal submit - reset confirmation for user ${interaction.user.id}, mode: ${mode}`);
    ...
    if (custom.startsWith('guess_submit:')) {
      ...
      logger.debug(`Processing guess_submit for gameId: ${gameId}`);
      const gameState = guessGames.get(gameId);
      logger.debug(`Game state found: ${!!gameState}, gameState type: ${typeof gameState}`);
      ...
      logger.debug(`Retrieved guess input: ${guess}, type: ${typeof guess}`);
    ...
    if (action === 'trivia') {
      ...
      if (gameState.currentQuestion >= gameState.questions.length) {
        ...
        try {
          ...
          logger.warn('Failed to update trivia achievements:', { error: error instanceof Error ? error.message : String(error) });
        }
