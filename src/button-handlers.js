...
import { getCharacter, encounterMonster, fightTurn, applyXp, saveCharacter, addItemToInventory, getItemInfo, generateRandomItem, getLeaderboardCount, randomEventType, getInventory, getInventoryValue } from './rpg.js';
...
\u{1F4A5} You take **${damage}** damage. HP: ${char2.hp}/${char2.maxHp}`;
      ...
      const embed = new EmbedBuilder()
        .setTitle('\u{1F3C3} Continue Adventure')
        .setColor(2_201_331)
        .setDescription(result)
        .addFields({ name: '\u{1F4CA} Stats', value: `Level ${char2.lvl} \u2022 XP ${char2.xp} \u2022 Gold ${char2.gold}`, inline: true });
    ...
    if (action === 'explore_leave') {
      ...
      const embed = new EmbedBuilder().setTitle('\u{1F3C3} Leave Location').setColor(16_750_592)
    ...
    if (action === 'music_radio_change') {
      ...
      try {
        ...
        if (!voiceChannel) {
          logCommandExecution(interaction, false, new Error('No voice channel'));
          return await safeInteractionReply(interaction, {
            content: '\u{1F3B5} You must be in a voice channel to change radio!',
            flags: MessageFlags.Ephemeral,
          });
        }
        const song = {
        ...
        if (!result.success) {
          ...
          const embed = new EmbedBuilder()
            .setTitle(`\u{1F4FB} Changed Station: ${station.name}`)
            .setColor(16_750_592)
            .setDescription(
              `**${station.name}** radio is now playing!

\u{1F3B5} *Live streaming activated*`,
            )
            .addFields(
      ...
      if (!nextLocationId) {
        ...
        if (unlockResult.success) {
          ...
          const successEmbed = new EmbedBuilder()
            .setTitle('\u{1F389} New Location Discovered!')
...
**Rewards:** ${unlockedLocation.rewards.xp} XP, ${unlockedLocation.rewards.gold} gold`,
      ...
      if (availableLocations.length === 0) {
        return safeInteractionReply(interaction, {
          content:
            '\u{1F3D5}\uFE0F No locations available yet. Start your adventure by exploring the Whispering Woods!\nUse `/explore discover location:whispering_woods`',
          flags: MessageFlags.Ephemeral,
        });
      }
      const locationOrder = ['whispering_woods', 'crystal_caverns', 'volcano_summit', 'forgotten_temple', 'shadow_realm', 'celestial_spire'];
...
${list}`);
      ...
      if (offsetNum > 0)
        row.addComponents(
          new ButtonBuilder()
            .setCustomId(`rpg_leaderboard:${Math.max(0, offsetNum - limit)}:${interaction.user.id}`)
            .setLabel('Prev')
      ...
      if (offsetNum + limit < total)
        row.addComponents(
          new ButtonBuilder()
            .setCustomId(`rpg_leaderboard:${offsetNum + limit}:${interaction.user.id}`)
            .setLabel('Next')
...
\u{1F4B0} You find **${gold}** gold!`;
      ...
      saveCharacter(interaction.user.id, char2);
      const embed = new EmbedBuilder()
        .setTitle('\u{1F50D} Investigation Results')
        .setColor(5_025_616)
        .setDescription(result)
        .addFields({ name: '\u{1F4CA} Stats', value: `Level ${char2.lvl} \u2022 XP ${char2.xp} \u2022 Gold ${char2.gold}`, inline: true });
      await safeInteractionUpdate(interaction, { embeds: [embed], components: [] });
...
\u{1F4A5} You take **${damage}** damage. HP: ${char2.hp}/${char2.maxHp}`;
      ...
      const embed = new EmbedBuilder()
        ...
        .setColor(event === 'trap' ? 16_711_680 : 2_201_331)
        .setDescription(result)
        .addFields({ name: '\u{1F4CA} Stats', value: `Level ${char2.lvl} \u2022 XP ${char2.xp} \u2022 Gold ${char2.gold}`, inline: true });
      await safeInteractionUpdate(interaction, { embeds: [embed], components: [] });
    }
    if (action === 'explore_rest') {
      ...
      try {
        ...
        saveCharacter(interaction.user.id, char2);
        const embed = new EmbedBuilder()
          .setTitle('\u{1F6CC} Rest Results')
          .setColor(5_025_616)
          .setDescription(
            `You take a peaceful rest in the safety of ${locationId}.
\u2764\uFE0F HP +${hpGain} \u2192 ${char2.hp}/${char2.maxHp}
\u{1F535} MP +${mpGain} \u2192 ${char2.mp}/${char2.maxMp}`,
          )
          .addFields({ name: '\u{1F4CA} Stats', value: `Level ${char2.lvl} \u2022 XP ${char2.xp} \u2022 Gold ${char2.gold}`, inline: true });
        await safeInteractionUpdate(interaction, { embeds: [embed], components: [] });
      } catch (error) {
...
\u{1F4A5} You take **${damage}** damage. HP: ${char3.hp}/${char3.maxHp}`;
        ...
        const embed = new EmbedBuilder()
          ...
          .setColor(2_201_331)
          .setDescription(result)
          .addFields({ name: '\u{1F4CA} Stats', value: `Level ${char3.lvl} \u2022 XP ${char3.xp} \u2022 Gold ${char3.gold}`, inline: true });
        await safeInteractionUpdate(interaction, { embeds: [embed], components: [] });
      }
      if (action === 'explore_leave') {
        const [, locationName, targetUserId2] = interaction.customId.split(':');
...
*Your adventure continues another day!*`);
      ...
      if (action === 'economy_business') {
        ...
        const embed = new EmbedBuilder().setTitle('\u{1F3EA} Business Income').setColor(5_025_616)
...
\u{1F4B0} New balance: **${balanceUpdate}** gold`);
      ...
      if (action === 'economy_invest') {
        ...
        const embed = new EmbedBuilder().setTitle('\u{1F4C8} Investment Results').setColor(profit > 0 ? 5_025_616 : 16_711_680)
...
`;
      ...
      if (action === 'trade_view_auctions') {
        ...
        const embed = new EmbedBuilder()
          .setTitle('\u{1F50D} Active Auctions')
          .setColor(2_201_331)
          .setDescription(
            auctions.length > 0
              ? auctions
                  .slice(0, 10)
                  .map((a) => `\u2022 ${a.itemName} x${a.quantity} - Starting: ${a.startingPrice} gold - Seller: ${a.seller}`)
                  .join('\n')
          ...
          if (firstCard.emoji === secondCard.emoji) {
            ...
            if (gameState.matchedPairs === gameState.totalPairs) {
              ...
              const winEmbed = new EmbedBuilder()
                .setTitle('\u{1F389} Memory Master!')
                .setDescription(
                  `Congratulations! You matched all ${gameState.totalPairs} pairs in ${gameState.moves} moves and ${timeElapsed} seconds! \u{1F3C6}`,
                )
                .setColor(65_280)
                .addFields(
                  {
...
**Efficiency:** ${((gameState.totalPairs / gameState.moves) * 100).toFixed(1)}%`,
      ...
      if (action === 'inventory_random') {
        ...
        const embed = new EmbedBuilder()
          .setTitle('\u{1F3B2} Random Item')
          .setColor(16_766_720)
          .setDescription(
            `You found a **${randomItem.name}**!

${randomItem.description}`,
          )
          .addFields({ name: '\u{1F4CA} Stats', value: `Rarity: ${randomItem.rarity} \u2022 Value: ${randomItem.value} gold`, inline: true });
        await safeInteractionUpdate(interaction, { embeds: [embed], components: [] });
      }
      if (action === 'inventory_sell_all') {
        ...
        saveCharacter(interaction.user.id, char3);
        const embed = new EmbedBuilder().setTitle('\u{1F4B0} Sold Junk Items').setColor(5_025_616)
...
\u{1F4B0} New balance: ${char3.gold} gold`);
      ...
      if (action === 'guild_refresh') {
        ...
        const embed = new EmbedBuilder()
          .setTitle('\u{1F504} Guild Refreshed')
          .setColor(5_025_616)
      ...
      if (action === 'party_invite') {
        ...
        const embed = new EmbedBuilder()
          .setTitle('\u{1F517} Party Invite Generated')
          .setColor(2_201_331)
      ...
      if (action === 'fun_rate') {
        ...
        try {
          ...
          const embed = new EmbedBuilder()
            ...
            .setColor(5_025_616)
            .setDescription(
              `You rated this joke with **${rating} star${rating !== 1 ? 's' : ''}!** \u2B50
        ...
        try {
          switch (content.type) {
            case 'story': {
              shareEmbed = new EmbedBuilder()
                .setTitle(`\u{1F4D6} ${content.genre.charAt(0).toUpperCase() + content.genre.slice(1)} Story`)
                .setColor(10_040_012)
            ...
            case 'fact': {
              shareEmbed = new EmbedBuilder()
                .setTitle(
                  `\u{1F9E0} ${content.category === 'random' ? 'Random' : content.category.charAt(0).toUpperCase() + content.category.slice(1)} Fun Fact`,
                )
                .setColor(5_025_616)
            ...
            case 'quote': {
              shareEmbed = new EmbedBuilder()
                .setTitle(`\u{1F4AC} ${content.category.charAt(0).toUpperCase() + content.category.slice(1)} Quote`)
                .setColor(15_277_667)
                .addFields(
          ...
          if (!interaction.channel) {
            const noChannelEmbed = new EmbedBuilder()
              ...
              .setColor(16_711_680)
              .setDescription('Channel not found to share content.')
...
${riddle.hint ? `*Hint: ${riddle.hint}*` : ''}`);
        const row = new ActionRowBuilder().addComponents(
          new ButtonBuilder()
            .setCustomId(`fun_riddle:${difficulty}:${riddle.id}:${interaction.user.id}`)
            .setLabel('\u{1F4A1} Show Answer')
            .setStyle(ButtonStyle.Primary),
          new ButtonBuilder()
            .setCustomId(`fun_riddle_new:${difficulty}:${interaction.user.id}`)
            .setLabel('\u{1F9E9} New Riddle')
            .setStyle(ButtonStyle.Secondary),
        );
        await safeInteractionUpdate(interaction, { embeds: [embed], components: [row] });
      }
...
${riddle.hint ? `*Hint: ${riddle.hint}*` : ''}`);
        const row = new ActionRowBuilder().addComponents(
          ...
          new ButtonBuilder()
            .setCustomId(`fun_riddle_new:${difficulty}:${interaction.user.id}`)
            .setLabel('\u{1F9E9} Another Riddle')
            .setStyle(ButtonStyle.Secondary),
        );
        await safeInteractionUpdate(interaction, { embeds: [embed], components: [row] });
      }
...
\u2014 ${quote.author || 'Unknown'}`);
      ...
      if (action === 'fun_challenge') {
        ...
        const row = new ActionRowBuilder().addComponents(
          ...
          new ButtonBuilder()
            .setCustomId(`fun_challenge_new:${type}:${interaction.user.id}`)
            .setLabel('\u{1F504} New Challenge')
            .setStyle(ButtonStyle.Secondary),
        );
        await safeInteractionUpdate(interaction, { embeds: [embed], components: [row] });
      }
      if (action === 'fun_challenge_new') {
        ...
        const challenge = createFunChallenge(type);
        const embed = new EmbedBuilder()
          .setTitle('\u{1F504} New Challenge')
          .setColor(16_729_344)
          .setDescription(String(challenge || 'No challenge available.'));
        const row = new ActionRowBuilder().addComponents(
          new ButtonBuilder().setCustomId(`fun_challenge:${type}:${interaction.user.id}`).setLabel('\u{1F3AF} Accept Challenge').setStyle(ButtonStyle.Primary),
          new ButtonBuilder()
            .setCustomId(`fun_challenge_new:${type}:${interaction.user.id}`)
            .setLabel('\u{1F504} Another Challenge')
            .setStyle(ButtonStyle.Secondary),
        );
        await safeInteractionUpdate(interaction, { embeds: [embed], components: [row] });
      }
      if (action === 'ai_chat') {
        ...
        const messageInput = new TextInputBuilder()
          ...
          .setRequired(true)
          .setPlaceholder('What would you like to say next?');
      ...
      if (action === 'achievements_refresh') {
        ...
        const embed = new EmbedBuilder()
          .setTitle('\u{1F504} Achievements Refreshed')
          .setColor(5_025_616)
      ...
      if (action === 'achievements_leaderboard') {
        ...
        if (!leaderboard || leaderboard.length === 0) {
          const embed2 = new EmbedBuilder()
            .setTitle('\u{1F3C5} Achievement Leaderboard')
            .setColor(16_766_720)
      ...
      if (action === 'ttt') {
        ...
        if (winner) {
          ...
          const resultEmbed = new EmbedBuilder()
            ...
            .setColor(winner === 'tie' ? 16_753_920 : 65_280)
            .setDescription(winner === 'tie' ? "\u{1F91D} **It's a tie!**" : `\u{1F389} **${gameState.players[winner].name} wins!**`)
            .addFields({
              name: 'Final Board',
        ...
        if (gameState.isAI && gameState.currentPlayer === 'O' && gameState.status === 'active') {
          ...
          if (aiMove !== null) {
            ...
            if (aiWinner) {
              ...
              const resultEmbed = new EmbedBuilder()
                .setTitle('\u2B55 Tic-Tac-Toe - Game Over!')
                .setColor(aiWinner === 'tie' ? 16_753_920 : 65_280)
                .setDescription(aiWinner === 'tie' ? "\u{1F91D} **It's a tie!**" : `\u{1F389} **${gameState.players[aiWinner].name} wins!**`)
                .addFields({
                  name: 'Final Board',
                  value: formatBoard(gameState.board),
                  inline: false,
      ...
      if (action === 'poll') {
        ...
        const updatedEmbed = new EmbedBuilder()
          .setTitle(`\u{1F4CA} ${pollData.question}`)
          .setColor(39_423)
          .setDescription(
            pollData.options
              .map((option, index) => {
                ...
                return (
                  ...
                  option +
                  '\n' +
                  '\u2588'.repeat(Math.max(1, percentage / 5)) +
                  (voteCount > 0 ? ' **' + voteCount + '** (' + percentage + '%)' : '')
                );
              })
              .join('\n\n'),
          )
...
Correct answer: **${currentQuestion.options[currentQuestion.correct]}**`,
          ...
          .setColor(isCorrect ? 65_280 : 16_711_680)
...
\u23F1\uFE0F Time: ${totalTime}s`,
            ...
            .setColor(percentage >= 70 ? 65_280 : percentage >= 50 ? 16_753_920 : 16_711_680)
            .setTimestamp();
          for (const [
            /** @type {number} */
            index,
...
${answer.isCorrect ? 'Your answer: ' + answer.userChoice : 'Your answer: ' + answer.userChoice + '\nCorrect: ' + answer.correctChoice}`,
          ...
          setTimeout(() => {
            ...
            const embed = new EmbedBuilder()
              .setTitle(`\u{1F9E0} Trivia Quiz - Question ${gameState.currentQuestion + 1}/${gameState.questions.length}`)
