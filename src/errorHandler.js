...
export async function safeExecuteCommand(interaction, commandFunction, context = {}) {
    try {
        ...
        if (error instanceof CommandError) {
            ...
            const commandError = new CommandError(error instanceof Error ? error.message : 'Unknown error occurred', 'UNKNOWN_ERROR', { originalError: error instanceof Error ? error.message : String(error), stack: error instanceof Error ? error.stack : undefined });
...
export function validatePermissions(interaction, permissions) {
    ...
    if (missingPermissions.length > 0) {
        throw new CommandError(`Missing permissions: ${missingPermissions.join(', ')}`, 'PERMISSION_DENIED', { required: permissions, missing: missingPermissions });
    }
}
/**
    ...
    for (let attempt = 1; attempt <= maxRetries; attempt++) {
        try {
            ...
            if (attempt === maxRetries) {
                throw new CommandError(`Operation failed after ${maxRetries} attempts: ${error instanceof Error ? error.message : String(error)}`, 'API_ERROR', { originalError: error instanceof Error ? error.message : String(error), attempts: maxRetries });
            }
