...
export const data = new SlashCommandBuilder()
  ...
  .addSubcommand((subcommand) => subcommand.setName('start').setDescription('Start a new Hangman game'))
  .addSubcommand((subcommand) =>
    subcommand
      .setName('guess')
      .setDescription('Guess a letter in the Hangman game')
      .addStringOption((option) => option.setName('letter').setDescription('The letter to guess').setRequired(true)),
  );
  ...
  return safeExecuteCommand(
    ...
    async () => {
      ...
      if (subcommand === 'start') {
        ...
        const currentDisplay = gameState.word
          .split('')
          .map((letter) => (gameState.guessedLetters.has(letter) ? letter : '_'))
          .join(' ');
