...
export const data = new SlashCommandBuilder()
  ...
  .addSubcommand((sub) =>
    sub
      ...
      .addStringOption((opt) => opt.setName('query').setDescription('Search topic').setRequired(false)),
  )
  ...
  .addSubcommand((sub) =>
    sub
      ...
      .addIntegerOption((opt) => opt.setName('number').setDescription('Number (1-1000, random if not specified)').setRequired(false)),
  )
  ...
  .addSubcommand((sub) =>
    sub
      .setName('github')
      .setDescription('Get GitHub user stats')
    ...
    switch (sub) {
      case 'news': {
        ...
        try {
          ...
          const embed = new EmbedBuilder().setTitle(`📰 Latest News: ${query}`).setColor(0x00_99_ff);
      ...
      case 'joke': {
        try {
          ...
          logger.error('Joke fetch error:', error);
          await interaction.reply({
            content: '❌ Failed to get joke. Please try again later.',
            flags: MessageFlags.Ephemeral,
          });
      ...
      case 'catfact': {
        try {
          ...
          logger.error('Cat fact fetch error:', error);
          await interaction.reply({
            content: '❌ Failed to get cat fact. Please try again later.',
            flags: MessageFlags.Ephemeral,
          });
      ...
      case 'numberfact': {
        ...
        try {
          ...
          logger.error('Number fact fetch error:', error);
          await interaction.reply({
            content: '❌ Failed to get number fact. Please try again later.',
            flags: MessageFlags.Ephemeral,
          });
      ...
      case 'dadjoke': {
        try {
          ...
          logger.error('Dad joke fetch error:', error);
          await interaction.reply({
            content: '❌ Failed to get dad joke. Please try again later.',
            flags: MessageFlags.Ephemeral,
          });
      ...
      case 'programquote': {
        try {
          ...
          logger.error('Programming quote fetch error:', error);
          await interaction.reply({
            content: '❌ Failed to get programming quote. Please try again later.',
            flags: MessageFlags.Ephemeral,
          });
      ...
      case 'github': {
        ...
        try {
          ...
          logger.error('GitHub stats fetch error:', error);
          await interaction.reply({
