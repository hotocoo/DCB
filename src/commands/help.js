    ...
    for (const file of fs.readdirSync(commandsPath)) {
      if (file.endsWith('.js')) {
        try {
          ...
          logger.error(`Failed to load command ${file}:`, error instanceof Error ? error : new Error(String(error)));
        }
      }
    }
  }
...
export async function execute(interaction) {
  ...
  for (const cmd of commands) {
    ...
    if (name === 'rpg') commandCategories.rpg.push(`\`/${name}\` - ${description}`);
      ...
      ['8ball', 'roll', 'rps', 'minigame', 'novel', 'connect4', 'guess', 'hangman', 'memory', 'tictactoe', 'trivia', 'wordle', 'fun', 'coinflip'].includes(name)
    )
      commandCategories.games.push(`\`/${name}\` - ${description}`);
    else if (['ping', 'echo', 'help', 'setmodel', 'togglechat', 'toggleplay', 'remind', 'poll', 'weather', 'music', 'profile'].includes(name))
      commandCategories.utility.push(`\`/${name}\` - ${description}`);
    else if (['chat', 'ai', 'api'].includes(name)) commandCategories.chat.push(`\`/${name}\` - ${description}`);
    else if (['admin', 'guild', 'achievements', 'economy', 'inventory', 'trade', 'explore'].includes(name))
      commandCategories.admin.push(`\`/${name}\` - ${description}`);
    else commandCategories.utility.push(`\`/${name}\` - ${description}`);
  }

  const embed = new EmbedBuilder().setTitle('🤖 Discord Bot Help').setColor(0x00_99_ff).setTimestamp();
