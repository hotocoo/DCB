...
import { safeExecuteCommand, CommandError, validateGuild, validateRange, validateNotEmpty } from '../errorHandler';
  ...
  switch (sub) {
    case 'me': {
      ...
      try {
        ...
        const timeUntil = Math.round((scheduledTime - Date.now()) / 60_000); // Convert to minutes
    ...
    case 'event': {
      ...
      try {
        ...
        const eventData = {
          ...
          duration: duration * 60_000, // Convert minutes to milliseconds
          channelId: interaction.channel.id,
        ...
        const timeUntil = Math.round((startTime - Date.now()) / 60_000);
