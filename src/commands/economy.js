...
export const data = new SlashCommandBuilder()
  ...
  .addSubcommand((sub) =>
    sub
      .setName('transfer')
      .setDescription('Transfer gold to another user')
  ...
  .addSubcommand((sub) =>
    sub
      ...
      .addStringOption((opt) => opt.setName('action').setDescription('buy|sell|prices').setRequired(true))
      .addStringOption((opt) => opt.setName('item').setDescription('Item to buy/sell'))
  ...
  .addSubcommand((sub) =>
    sub
      ...
      .addIntegerOption((opt) => opt.setName('ticket_price').setDescription('Ticket price').setRequired(true)),
  )
  .addSubcommand((sub) =>
    sub
      .setName('history')
      .setDescription('Transaction history')
      .addIntegerOption((opt) => opt.setName('limit').setDescription('Number of transactions')),
  )
    ...
    async () => {
      ...
      switch (sub) {
            ...
            case 'list': {
              ...
              if (businesses.length === 0) {
                return interaction.reply({
                  content: '🏢 You have no businesses yet. Use `/economy business action:create` to start one!',
                  flags: MessageFlags.Ephemeral,
                });
              }

              const embed = new EmbedBuilder().setTitle('🏢 Your Businesses').setColor(0xff_d7_00).setDescription('Manage your business empire!');
        ...
        case 'market': {
          ...
          switch (action) {
            case 'prices': {
              const embed = new EmbedBuilder().setTitle('📈 Market Prices').setColor(0x00_99_ff);
        ...
        case 'history': {
          ...
          if (transactions.length === 0) {
            const embed = new EmbedBuilder().setTitle('📋 Transaction History').setColor(0xff_a5_00).setDescription('No transaction history found.');
