...
export const data = new SlashCommandBuilder()
  ...
  .addSubcommand((sub) =>
    sub
      .setName('analyze')
      .setDescription('Analyze text sentiment')
      .addStringOption((opt) => opt.setName('text').setDescription('Text to analyze').setRequired(true)),
  )
  .addSubcommand((sub) =>
    sub
      .setName('summarize')
      ...
      .addIntegerOption((opt) => opt.setName('length').setDescription('Max summary length').setRequired(false)),
  )
  .addSubcommand((sub) =>
    sub
      .setName('translate')
  ...
  .addSubcommand((sub) =>
    sub
      .setName('ideas')
      .setDescription('Generate ideas')
      .addStringOption((opt) => opt.setName('topic').setDescription('Topic for ideas').setRequired(true))
      .addIntegerOption((opt) => opt.setName('count').setDescription('Number of ideas').setRequired(false)),
  )
  .addSubcommand((sub) =>
    sub
      .setName('code')
      ...
      .addStringOption((opt) => opt.setName('description').setDescription('Code description').setRequired(true)),
  )
    ...
    switch (sub) {
      case 'chat': {
        ...
        try {
          ...
          const embed = new EmbedBuilder()
            ...
            .addFields({ name: 'Model', value: model, inline: true }, { name: 'Personality', value: personality, inline: true });
        ...
        try {
          ...
          const embed = new EmbedBuilder()
            ...
            .addFields(
              ...
              {
                name: `Translated (${language.toUpperCase()})`,
                value: translation.length > 1024 ? translation.slice(0, 1021) + '...' : translation,
                inline: false,
              },
            );
      ...
      case 'models': {
        try {
          ...
          const embed = new EmbedBuilder().setTitle('🤖 Available AI Models').setColor(0x00_99_ff);
      ...
      case 'personalities': {
        try {
          ...
          const embed = new EmbedBuilder().setTitle('🎭 Available Personalities').setColor(0xe9_1e_63);
