...
async function handleWin(interaction, gameState) {
  const { guesses, maxGuesses, startTime, difficulty, secretWord } = gameState;
  ...
  const winEmbed = new EmbedBuilder()
    ...
    .setColor(0x00_ff_00)
    .setDescription(`You guessed **${secretWord}** in ${guesses.length}/${maxGuesses} attempts!\n\n` + performanceMessage(guesses.length))
    .addFields({
      name: '📊 Game Stats',
...
async function sendActiveBoard(interaction, gameState) {
  ...
  if (guesses.length > 0) {
    const guessHistory = guesses.map((guess, index) => `**${index + 1}.** ${guess.word}\n${resultToString(guess.result)}`).join('\n\n');
  ...
  const guessBtn = new ButtonBuilder().setCustomId(`wordle_guess:${interaction.user.id}`).setLabel('🔤 Make Guess').setStyle(ButtonStyle.Primary);
