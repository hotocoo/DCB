...
export const data = new SlashCommandBuilder()
  ...
  .addSubcommand((sub) =>
    sub
      .setName('create')
      .setDescription('Create a new novel')
      .addStringOption((opt) => opt.setName('title').setDescription('Title'))
      .addStringOption((opt) => opt.setName('prompt').setDescription('Initial prompt/context')),
  )
  .addSubcommand((sub) => sub.setName('list').setDescription('List novels'))
  .addSubcommand((sub) =>
    sub
      .setName('read')
      .setDescription('Read a novel')
      .addStringOption((opt) => opt.setName('id').setDescription('Novel ID').setRequired(true))
      .addIntegerOption((opt) => opt.setName('chapter').setDescription('Chapter number')),
  )
  .addSubcommand((sub) =>
    sub
      .setName('next')
      .setDescription('Generate next chapter')
      .addStringOption((opt) => opt.setName('id').setDescription('Novel ID').setRequired(true)),
  );
...
export async function execute(interaction) {
  try {
    ...
    if (sub === 'list') {
      ...
      const embed = new EmbedBuilder().setTitle('📚 Available Novels').setColor(0x00_99_ff).setDescription(novels.join('\n\n'));
